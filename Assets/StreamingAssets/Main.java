...
// class Player {
//     String name;
//     int energy, xp, coins, positionX, positionY, increasedBackpackDuration, dazeTurns, frozenTurns, backpackCapacity,
//             rawMinerals, processedMinerals, rawDiamonds, processedDiamonds;

//     public Player(String name, int energy, int xp, int coins, int positionX, int positionY,
//             int increasedBackpackDuration, int dazeTurns, int frozenTurns, int backpackCapacity, int rawMinerals,
//             int processedMinerals, int rawDiamonds, int processedDiamonds) {
//         this.name = name;
//         this.energy = energy;
//         this.xp = xp;
//         this.coins = coins;
...
public class Main {
    ...
    // System.out.println("mine " + nextToDiamondsPos[0] + " " +
    // nextToDiamondsPos[1]);
    // return;
    // }
    // List<int[]> closestDiamond = findDiamonds(board, player);
    // if (closestDiamond != null) {
    // int[] nextMoveToDiamond = nextMove(closestDiamond, player.positionX,
    // player.positionY);
    // if (nextMoveToDiamond != null) {
    // System.out.println("move " + nextMoveToDiamond[0] + " " +
    // nextMoveToDiamond[1]);
    // } else {
    // System.out.println("rest");
    // }
    ...
    // } else {
    // if (myX == 0 && myY == 11) {
    // System.out.println("conv 0 diamond 0 mineral to coins, 0 diamond 0 mineral to
    // energy, " + player.rawDiamonds + " diamond " + player.rawMinerals + " mineral
    // to xp");
    // return;
    // }
    ...
    // if (closestBase != null) {
    // int[] nextMoveToBase = nextMove(closestBase, player.positionX,
    // player.positionY);
    ...
    // if (i == 0 && j == 0) continue;
    // if (!(i == 0 || j == 0)) continue;
    // if (!(myX + i >= 0 && myX + i < board.grid.length && myY + j >= 0 && myY + j
    // < board.grid[0].length)) continue;
    ...
    // static List<int[]> findPathForTarget(Board board, Player player, int[]
    // targetPos) {
    // int myX = player.positionX;
    // int myY = player.positionY;
    ...
    // while (!queue.isEmpty()) {
    // int[] pos = queue.poll();
    // if (pos[0] == targetPos[0] && pos[1] == targetPos[1]) return
    // reconstructPath(visited, targetPos);
    // if (visited.contains(Arrays.toString(pos))) continue;
    // visited.add(Arrays.toString(pos));
    // if (board.grid[pos[0]][pos[1]] != 'E' && board.grid[pos[0]][pos[1]] != '2')
    // continue;
    // int[][] directions = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    // for (int[] direction : directions) {
    // int newX = pos[0] + direction[0];
    // int newY = pos[1] + direction[1];
    // if (!(newX >= 0 && newX < board.grid.length && newY >= 0 && newY <
    // board.grid[0].length)) continue;
    ...
    // int[] directionNextStep = {nextStep[0] - path.get(i - 1)[0], nextStep[1] -
    // path.get(i - 1)[1]};
    // if (!Arrays.equals(directionNextStep, directionFirstStep)) return path.get(i
    // - 1);
    // }
